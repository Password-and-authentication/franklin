...
// insert item at head
#define SLIST_INSERT_HEAD(item, head, field) do {		\
    (item)->field.next = (head)->first;				\
    (head)->first = (item);					\
} while(0)

  // insert an item after any item
#define SLIST_INSERT_AFTER(item, prev, field) do {			\
    (prev)->field.next = (item);				\
    (prev) = (prev)->field.next;				\
  } while(0)

// remove head from a list
#define SLIST_REMOVE_HEAD(item, head, field) do {	\
    (item) = (head)->first;					\
...
#define SLIST_REMOVE_ITEM(item, head, type, field) do {	\
    if (item == (head)->first) {			\
...
#define SLIST_FOREACH(item, head, field)	\
  for (item = SLIST_FIRST((head));		\
       item;					\
       item = SLIST_NEXT(item, field)		\
  )
