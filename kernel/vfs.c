...
struct vnode {
  struct vnodeops *op; // vnode operations (open, read, write etc)
  struct vfs *vfs; // vfs the vnode is in
  struct vfs *vfsmountedhere; // incase vnode is mount point
  uint16_t count; // ref count
  void *data;  // pointer to fs specific data
};
...
struct vfs {
  ...
  struct vnode *vnodecovered; // mount point
  /* uint8_t flags; */
  /* int bsize; */
  void* data; // pointer to fs specific data
};
